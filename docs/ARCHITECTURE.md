...
`./scripts/lifecycle/up.sh` still starts **Kind first**, then LocalStack, then bridges
...
1. Start **LocalStack alone** (`SKIP_KIND=1 ./scripts/lifecycle/up.sh`) + latency smoke  
...
Local `./scripts/lifecycle/up.sh` flow (unchanged convenience):
...
- `BACKEND=s3` — S3 bucket `tfstate-<project>-<env>` + DynamoDB lock table
  `tflock-<project>-<env>` on LocalStack (bootstrap via `./scripts/backend/use-s3-backend.sh`)
...
All three tiers are generated by `scripts/lifecycle/sync-live.sh` from `terraform/live/_templates` (same modules/shape). Production differs only via tfvars knobs above (longer retention, stricter SQS depth, longer secret recovery) plus naming/CIDR — not divergent Terraform code paths.
...
| **2. Grafana + Loki + Alloy** | OSS log UI for Kind/Docker containers. | Opt-in: `./scripts/observability/observability-up.sh` → `docker-compose.observability.yml`. **Never** started by `up.sh` or CI. |
| **3. Portainer CE** | Docker + Kubernetes debug UI (`profiles: ["debug"]`). | Opt-in: `./scripts/debug/portainer-up.sh`. **Never** started by `up.sh` or CI. |
...
```bash
./scripts/observability/observability-up.sh    # Grafana http://localhost:3000 (admin/admin)
./scripts/observability/observability-down.sh  # down -v
./scripts/debug/portainer-up.sh                # Portainer http://localhost:9000
./scripts/debug/portainer-down.sh              # stop only (keeps admin volume)
```
...
Workflow: [`.github/workflows/drift-check.yml`](../.github/workflows/drift-check.yml) (`cron` daily + `workflow_dispatch`). Shared logic: [`scripts/checks/check-drift.sh`](../scripts/checks/check-drift.sh) (also called from `verify-apply.sh`).
...
**CI sequencing (same as `terraform.yml`):** LocalStack alone → S3 bootstrap → apply `shared/network/backend` while host `:4566` works → Kind → apply `eks`. On Linux, attaching LocalStack to the Kind network can break published `:4566`; `scripts/lifecycle/env.sh` then switches the provider **and** re-bakes S3 `backend` block endpoints to the container IP (`init -reconfigure`). Sibling `terraform_remote_state` reads use `var.localstack_endpoint` (not a baked URL) so the same switch covers network/backend state fetches during `eks` apply. Default `terraform.yml` stays on `BACKEND=local` and is unaffected.
...
**Local (`./scripts/lifecycle/up.sh`):** Kind → LocalStack → full apply.
...
`scripts/lifecycle/env.sh` then switches `TF_VAR_localstack_endpoint` to LocalStack's
