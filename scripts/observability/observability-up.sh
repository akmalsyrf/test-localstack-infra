...
# Start optional Grafana + Loki + Alloy (local-dev only; never used by CI).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$ROOT"

echo "==> Starting observability stack (Grafana OSS / Loki / Alloy)..."
...
for _ in $(seq 1 60); do
  if curl -sf http://localhost:3000/api/health >/dev/null 2>&1; then
    ...
    echo "  Alloy:    collector (docker logs → Loki); UI :12345 inside compose network"
    echo "  Datasource Loki is provisioned as default; open Explore or the"
    echo "  'Kind / Docker logs (local-dev)' dashboard."
    echo ""
    echo "NOTE: This stack uses meaningful CPU/RAM. Stop with ./scripts/observability/observability-down.sh"
    echo "when finished. Not started by ./scripts/lifecycle/up.sh or any CI workflow."
    exit 0
  fi
  sleep 2
done
